export const entry = {
  ...
  problem: `In a standard ("dense") Transformer, all parameters are used for every input. If a model has 70B parameters, all 70B process the word "the." Computational cost scales directly with total parameter count — to make a model smarter, you need proportionally more compute per token.
...
Mixture of Experts breaks this link between total parameters and per-token cost. Instead of one large feed-forward network in each Transformer layer, MoE uses several smaller "expert" sub-networks. A gating function (a small learned router) examines each token and sends it to just 1–2 of the available experts. A model can have, say, 8x7B = 56B total parameters but only activate 7B per token — the same cost as a regular 7B model, but with access to far more specialized knowledge. The engineering challenge is load-balancing: making sure all experts get used roughly equally, so no expert is wasted and no expert is overloaded.`,
  ...
  howInvented: `MoE was invented by reviving an older ensemble idea inside modern sequence models: let a learned router choose which sub-network handles each token instead of firing the whole model. Independent convergence: about 2 major groups were pushing sparse-expert systems at scale in the Transformer era, with Google's Shazeer/GShard/Switch line being the clearest throughline.`,
