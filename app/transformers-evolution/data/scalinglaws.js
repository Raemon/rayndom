export const entry = {
  ...
  problem: `Before 2020, training a large language model was largely guesswork: researchers chose a model size, gathered as much data as seemed reasonable, and trained until results looked good. There was no principled way to predict how a model would perform before spending millions of dollars to train it. The relationship between model size, dataset size, compute budget, and final performance was unknown.
...
This transformed model development from an empirical art into something closer to engineering. Labs could now plan training runs rationally: given a fixed compute budget, the scaling laws prescribed an optimal model size. This predictability justified the hundred-million-dollar investments behind GPT-3 and its successors. However, the paper's specific recommendations about how to allocate compute between model size and data turned out to be significantly wrong — a correction Chinchilla would later provide.`,
  whyNotSooner: `Required training dozens of models at multiple scales and fitting precise curves — expensive experiments few labs could run. The idea that a simple power-law formula could predict performance of a system as complex as a language model was not obvious a priori.`,
  examples: "Kaplan et al. 'Scaling Laws for Neural Language Models' (2020),Justified GPT-3's 175B parameter count,Foundation for all compute-allocation decisions at frontier labs",
};
