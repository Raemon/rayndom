export const entry = {
  ...
  problemOneLiner: `Handwritten rules could not scale, so the perceptron learned weighted decision boundaries directly from examples.`,
  problem: `Before 1958, if you wanted a machine to recognize a handwritten letter, you had to write out every rule yourself. There was no way for a machine to learn patterns from data on its own.
...
The fatal limitation: a perceptron can only draw a straight line to separate categories (this property is called "linear separability"). If the data needs a curved boundary — or even something as simple as XOR ("true when exactly one input is on") — a single perceptron cannot learn it. This limit, proven mathematically by Minsky and Papert in 1969, froze the field for over a decade.`,
  whyNotSoonerOneLiner: `The needed neuron math, learning rules, and even barely adequate hardware were only just appearing.`,
