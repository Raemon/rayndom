export const entry = {
  ...
  problem: `Models generated text but couldn't act, so agent loops let them call tools, observe results, and re-plan.

No matter how capable, a language model fundamentally just produces a string of characters — any actual effect on the world requires a human to take action.
...
The reason this only became practical in 2024–25 is reliability: agentic tasks chain multiple steps, and errors compound. If each step has 95% accuracy, a 10-step task succeeds only 60% of the time. Instruction following, structured output generation, and long context — all prerequisites — only reached sufficient quality in 2023–24.`,
  whyNotSooner: `Chained tool use needed near-perfect structured output, long context, and instruction following before loops were reliable.

Model capability — instruction following, structured output, long context — only reached sufficient quality in 2023–24. Reliable tool use requires ~99% per-step accuracy — 95% compounds to 60% over 10 steps. Safety infrastructure also needed to mature.`,
