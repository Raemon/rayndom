export const entry = {
  ...
  problem: `Even with FlashAttention, every token must attend to every other token. Doubling sequence length quadruples computation. For very long sequences (100K+ tokens), this is a fundamental hardware bottleneck.
...
Hybrid architectures get the best of both by interleaving SSM layers (for efficient long-range processing) with sparse attention layers (for precise information retrieval). Models like Mamba introduced selective gating, which lets the SSM dynamically decide what to store in its state based on input content, closing much of the quality gap with full attention.`,
  ...
  howInvented: `Modern SSM hybrids were invented by revisiting classical state-space models with deep-learning tooling, then adding selective gating and hardware-aware kernels so they could compete on language tasks. Independent convergence: about 3 lines were converging — RWKV-style recurrent-transformer hybrids, Mamba-style selective SSMs, and hybrid architectures like Jamba that mix SSM and attention blocks.`,
