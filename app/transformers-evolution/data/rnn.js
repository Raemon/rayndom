export const entry = {
  ...
  problem: `Feedforward nets forgot order entirely, so recurrence carried a hidden state forward as sequence memory.

Feedforward networks processed each input independently — they had no notion of order. Feed in the word "bank" and the network has no idea whether it appeared after "river" or "savings." For tasks like speech or language, where meaning depends on what came before, this was a fundamental limitation.
...
The limitation was practical: training requires "unrolling" the loop across all timesteps and running backpropagation through the full chain (called BPTT — Backpropagation Through Time). Over more than ~10–20 steps, gradients either vanish (shrink to near-zero) or explode (grow uncontrollably), making it impossible to learn long-range dependencies.`,
  whyNotSooner: `The architecture needed BPTT to formalize learning through time, and early hardware made sequence training expensive.
