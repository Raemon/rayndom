export const entry = {
  ...
  problem: `Once models could reliably call tools, a new category of problems emerged — not in the model itself, but in the engineering infrastructure around it. Three distinct failure modes became clear as tool-using agents tackled real multi-step tasks:
...
Persistent memory across sessions: most agents are stateless between sessions — each conversation starts from zero. OpenClaw introduced persistent file-based memory: SOUL.md (agent identity and personality), MEMORY.md (long-term facts learned from past sessions), and guardrails.md (lessons about what to avoid). Combined with a model-agnostic runtime, this enables agents that accumulate knowledge across runs rather than starting fresh each time.`,
  whyNotSooner: `Required tool-using models to be reliable enough that scaffolding — not model capability — became the bottleneck. Also required enough real-world autonomous agent deployment to discover these failure modes empirically. RALPH emerged from practitioners running agents for hours and watching context degradation firsthand.`,
