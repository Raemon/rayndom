export const entry = {
  ...
  problem: `Long-running agents lost track of state, so scaffolds moved memory, plans, and resets into external runtime loops.

Once models could call tools, a new category of problems emerged — not in the model itself, but in the engineering around it. Three distinct failure modes became clear:
...
Persistent memory across sessions: most agents are stateless between sessions — each conversation starts from zero. OpenClaw introduced persistent file-based memory: SOUL.md (agent identity and personality), MEMORY.md (long-term facts learned from past sessions), and guardrails.md (lessons about what to avoid). Combined with a model-agnostic runtime, this enables agents that accumulate knowledge across runs rather than starting fresh each time.`,
  whyNotSooner: `These failure modes only appeared once tool-using agents were good enough for multi-hour real-world runs.

Required tool-using models to be reliable enough that scaffolding — not model capability — became the bottleneck. RALPH emerged from practitioners running agents for hours and watching context degradation firsthand.`,
