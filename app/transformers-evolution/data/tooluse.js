export const entry = {
  ...
  problem: `Text-only outputs could describe actions but not take them, so function calls let models request external tools.

No matter how capable, a language model fundamentally just generates characters. It can write code but not run it, describe a web search but not perform one. The boundary between "thinking" and "acting" required a human to bridge.
...
The key enablers were reliable structured output and instruction following. Without near-perfect JSON formatting, tool calls fail silently — a malformed function call doesn't produce a helpful error, it just does the wrong thing. The difference between 95% and 99.5% reliability in structured output is the difference between an unusable system and a practical one, because agentic tasks chain multiple steps and errors compound: 95% per step over 10 steps gives only 60% end-to-end success.`,
  whyNotSooner: `Reliable JSON, instruction following, and the mindset shift from chatbot to agent all had to arrive first.

Without near-perfect structured output formatting, tool calls fail silently. The conceptual shift from treating LLMs as text generators to action-taking agents was also required.`,
