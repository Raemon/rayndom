...
export async function POST(request: NextRequest) {
  ...
  const timeblock = await prisma.timeblock.create({ data: { datetime: new Date(datetime), rayNotes: body?.rayNotes ?? null, assistantNotes: body?.assistantNotes ?? null } })
...
export async function PATCH(request: NextRequest) {
  ...
  const timeblock = await prisma.timeblock.update({ where: { id: Number(id) }, data: { rayNotes: body?.rayNotes ?? undefined, assistantNotes: body?.assistantNotes ?? undefined } })
  return NextResponse.json({ timeblock })
}
