...
const OutdoorRentalsPage = ({venues}: {venues: Venue[]}) => {
  ...
  const mapItems = venues.map(v => ({
    ...
    Location: v.Location, Type: v.Type, Capacity: v.Capacity,
              ...
              <div>
                ...
                {images.length > 0 && (
                  <div style={{display: 'flex', gap: '2px', marginTop: '4px', flexWrap: 'wrap'}}>
                    {images.map((url, i) => (
                      <img key={i} src={url} alt="" style={{width: '48px', height: '36px', objectFit: 'cover'}}
                ...
                return (
                  <div key={venue.Name} onClick={() => setSelectedVenue(venue)}
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-400 truncate">{venue.Type} · {venue.Capacity}</div>
                    </div>
                  </div>
                )
              })}
