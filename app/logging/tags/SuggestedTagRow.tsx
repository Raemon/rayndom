...
const SuggestedTagRow = ({ tag, counts, onClick, isSelected, onMouseEnter, onMouseLeave, onContextMenu }:{
  ...
  return (
    <div className={`flex items-center gap-2 text-sm  ${onClick ? ' cursor-pointer hover:bg-white/10' : ''}`} onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onContextMenu={e => { e.preventDefault(); e.stopPropagation(); onContextMenu?.() }}>
      ...
      {isSelected && tag.suggestedTagIds?.length ? <span className="text-white/40 text-3xl leading-none pointer-events-none">›</span> : null}
    </div>
  )
}
