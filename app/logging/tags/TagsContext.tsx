...
type TagsContextType = {
  ...
  updateTag: (args: { id: number, name?: string, type?: string, parentTagId?: number | null, suggestedTagIds?: number[] | null }) => Promise<void>
...
export const TagsProvider = ({ children }:{ children: ReactNode }) => {
  ...
  const updateTag = useCallback(async ({ id, name, type, parentTagId, suggestedTagIds }:{ id: number, name?: string, type?: string, parentTagId?: number | null, suggestedTagIds?: number[] | null }) => {
    setTags(prev => prev.map(t => t.id === id ? { ...t, name: name ?? t.name, type: type ?? t.type, parentTagId: parentTagId !== undefined ? parentTagId : t.parentTagId, suggestedTagIds: suggestedTagIds !== undefined ? suggestedTagIds : t.suggestedTagIds } : t))
    const res = await fetch('/api/timer/tags', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, name, type, parentTagId, suggestedTagIds }) })
    const json = await res.json()
