  ...
  return (
    <div className="flex flex-col items-start gap-2" onKeyDown={handleKeyDown}>
      <div className="flex gap-2 w-full">
        <input className="px-2 py-1 bg-gray-700 text-white outline-none flex-1 text-sm!" value={name} onChange={(e) => setName(e.target.value)} placeholder="Name" autoFocus />
        <input className="px-2 py-1 bg-gray-700 text-white outline-none w-28 text-sm!" value={type} onChange={(e) => setType(e.target.value)} placeholder="Type" />
        <input className="px-2 py-1 bg-gray-700 text-white outline-none w-24 text-sm!" value={subtype} onChange={(e) => setSubtype(e.target.value)} placeholder="Subtype" />
      </div>
      <textarea className="px-2 py-1 bg-gray-700 text-white outline-none w-full text-sm! resize-none [field-sizing:content]" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description (for AI tag prediction)" />
