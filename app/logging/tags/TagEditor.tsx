...
const TagEditor = ({ tag, onSave, onDelete, onCancel }:{ tag: Tag, onSave: (args: { id: number, name: string, type: string, description: string | null }) => Promise<void> | void, onDelete: (args: { id: number }) => Promise<void> | void, onCancel?: () => void }) => {
  ...
  return (
    <div className="flex flex-col items-start gap-2" onKeyDown={handleKeyDown}>
      ...
      <input className="px-2 py-1 bg-gray-700 text-white outline-none w-full text-xs!" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description (for AI tag prediction)" />
      <div className="flex gap-2">
        <button className="px-2 py-1 bg-white/10 hover:bg-white/20 text-white text-xs! cursor-pointer" onClick={() => onSave({ id: tag.id, name, type, description: description || null })}>Save</button>
