...
export const useCommands = ({ autoLoad=true }:{ autoLoad?: boolean } = {}) => {
  ...
  const load = async () => {
    const res = await fetch('/api/logging/commands')
  ...
  const createCommand = async ({ name, html }:{ name: string, html: string }) => {
    ...
    const res = await fetch('/api/logging/commands', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, html }) })
    const json = await res.json()
  ...
  const updateCommand = async ({ id, name, html }:{ id: number, name?: string, html?: string }) => {
    ...
    const res = await fetch('/api/logging/commands', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, name, html }) })
    const json = await res.json()
  ...
  const deleteCommand = async ({ id }:{ id: number }) => {
    ...
    await fetch(`/api/logging/commands?id=${id}`, { method: 'DELETE' })
  }
