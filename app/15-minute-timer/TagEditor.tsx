...
const TagEditor = ({ tag, onSave, onDelete }:{ tag: Tag, onSave: (args: { id: number, name: string, type: string }) => Promise<void> | void, onDelete: (args: { id: number }) => Promise<void> | void }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button className="px-2 py-1 bg-gray-200" onClick={() => onSave({ id: tag.id, name, type })}>Save</button>
      <button className="px-2 py-1 bg-gray-200" onClick={() => onDelete({ id: tag.id })}>Del</button>
    </div>
  )
}
